...
void Upstairs::execute(BipedalController* controller, const ros::Time& time, const ros::Duration& period)
{
  ...
  double theta_des_l{ M_PI_2 - 0.4 }, theta_des_r{ M_PI_2 - 0.4 }, length_des_l{ 0.18 }, length_des_r{ 0.18 };
  ...
  // Exit
  if ((left_pos_[0] < 0.2 && left_pos_[1] > (M_PI_2 - 0.5) && abs(left_spd_[1]) < 0.1) &&
      (right_pos_[0] < 0.2 && right_pos_[1] > (M_PI_2 - 0.5) && abs(right_spd_[1]) < 0.1))
  {
